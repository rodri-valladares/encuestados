...
var VistaUsuario = function(modelo, controlador, elementos) {
  ...
  this.modelo.paginaIniciada.suscribir(function(){
    contexto.reconstruirGrafico()
  });
};
...
VistaUsuario.prototype = {
  ...
  reconstruirLista: function() {
    ...
    preguntas.forEach(function(clave){
      ...
      //*MIO-Para mostrar los titulos y mas de las preguntas creadas en administrador.
      nuevoElemento = $("<div id="+`${clave.id}`+" value="+`${clave.textoPregunta}`+">"+`${clave.textoPregunta.toUpperCase()}`+"</div>");
      listaPreguntas.append(nuevoElemento);//*MIO
      var respuestas = clave.cantidadPorRespuesta;
  ...
  agregarVotos: function(){
    ...
    $('#preguntas').find('div').each(function(){
       // var nombrePregunta = $(this).attr('value'); el codigo original traia esta linea , 
       //no permitia que sumarVoto en modelo funcione correctamente. Se corrigio con $(this).text()
